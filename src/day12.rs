...
fn possible_arrangements(report: &[char], remaining_regions: &[usize], memo: &mut Memo) -> usize {
	...
	&& report[0..remaining_regions[0]].iter().all(|&ch| ch == '#' || ch == '?') 
	&& (report.len() == remaining_regions[0] || report[remaining_regions[0]] != '#') {
		result += possible_arrangements(&report[remaining_regions[0] + 1..], &remaining_regions[1..], memo);
	}
...
pub fn solve(inputs: Vec<String>) {
	let springs_and_regions = inputs.iter().map(|line| {
		let (report, damaged_regions) = line.split_once(" ").unwrap();
		let damaged_regions = damaged_regions.split(",").map(|s| s.parse::<usize>().unwrap()).collect_vec();

		(report, damaged_regions)
	}).collect_vec();

	let part1 = springs_and_regions.iter().map(|(springs, regions)| {
	...
	let part2 = springs_and_regions.iter().map(|(springs, regions)| {
