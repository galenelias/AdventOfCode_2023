...
pub fn solve(inputs: Vec<String>) {
	...
	let start_pipe_neighbors = get_adjacents(&start_pos)
		.into_iter()
		.filter(|p| {
			let ch = grid[p.0][p.1];
	...
	let start_pipe_piece = ['|', '-', 'L', 'J', '7', 'F']
		.iter()
		.find(|&ch| {
			let pipe_ends = get_pipe_ends(ch, &start_pos);
			return (pipe_ends[0] == start_pipe_neighbors[0]
				&& pipe_ends[1] == start_pipe_neighbors[1])
				|| (pipe_ends[1] == start_pipe_neighbors[0]
					&& pipe_ends[0] == start_pipe_neighbors[1]);
		})
		.unwrap();
		...
		if pos.1 == prev_pos.1 - 1 {
			...
			if pos.0 == prev_pos.0 - 1 {
				...
				match ch {
					...
					_ => {
						panic!("Unknown pipe type: {}", ch);
					}
				}
			} else if pos.0 == prev_pos.0 + 1 {
				// Moving down
				match ch {
					'J' => {
						// turning left
